...
const generateAuthSOAPRequest = (username: string, password: string): string => {
...
export const performSOAPAuthentication = async (username: string, password: string) => {
  ...
  try {
    ...
    const fedAuth = response.headers?.['set-cookie']?.find(
        (cookie: string) => cookie.startsWith('FedAuth')
      ) ?? '';
